...
HTTP_PROXY_REQUIRED = 305,
    ...
    HTTPS_RE = /^https\:?/i,
    REQUEST_OPTIONS = ['agent', 'auth', 'family', 'headers', 'host', 'hostname', 'json', 'localAddress', 'maxRetries', 'method', 'path', 'pathname', 'port', 'protocol', 'proxy', 'query', 'rejectUnauthorized', 'maxRetries', 'rawStream', 'secure', 'socketPath', 'timeout'],
...
function _exec(options, data, tryCount, callback) {
	...
	exec = new Promise(function (resolve, reject) {
		...
		if (!_core2.default.Validation.isEmpty(options.proxy)) {
			...
			if (options.path.indexOf(host) < 0) {
				...
				options.path = [options.secure ? 'https' : 'http', _path2.default.join(host, options.path)].join('://');
			}
